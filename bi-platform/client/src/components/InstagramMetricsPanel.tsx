...
import { BarChart3, ExternalLink, MousePointerClick, Radar, Users, Video } from "lucide-react";
...
export default function InstagramMetricsPanel() {
  const metricsQuery = trpc.instagram.dailyMetrics.useQuery();
  ...
  const metrics = [
    ...
    { label: "Alcance", value: latest?.reach, icon: Radar },
  ...
  return (
    <Card className="mb-6 border-border/70 bg-card/90 shadow-sm">
      <CardHeader className="flex flex-row items-start justify-between gap-4 space-y-0">
        <div>
          <CardTitle className="font-serif text-xl text-card-foreground">Instagram · panorama real</CardTitle>
      ...
      <CardContent>
        {metricsQuery.isLoading ? (
          ...
          <p className="rounded-xl bg-destructive/10 p-4 text-sm text-destructive">Não foi possível carregar as métricas protegidas do Instagram.</p>
        ) : (
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
