...
// Labels exatamente como o servidor grava em social_followers (server/lib/followerSheetSync.ts PLATFORMS)
const PLATFORM_LABELS: Record<Props["platform"], string> = {
  instagram: "Instagram",
  tiktok: "TikTok",
  facebook: "Facebook",
  youtube: "YouTube",
...
export default function FollowerGrowth({ platform, currentFollowers }: Props) {
  ...
  const rows = useMemo<Row[]>(() => {
    const normalizedPlatform = PLATFORM_LABELS[platform].toLowerCase();
    return (followerHistory.data ?? [])
      .filter(row => row.platform.toLowerCase() === normalizedPlatform)
      .map(row => ({ date: row.observedDate, followers: row.followerCount, new_followers: 0 }))
      .sort((a, b) => a.date.localeCompare(b.date));
  }, [followerHistory.data, platform]);
  ...
  const stats = useMemo(() => {
    ...
    const findOnOrBefore = (targetTs: number) => {
      ...
      for (const r of withFollowers) {
        const t = parseDateOnly(r.date).getTime();
    ...
    const lastTs = last ? parseDateOnly(last.date).getTime() : Date.now();
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-[11px] text-muted-foreground">
            Base: Google Sheet · Log Diário{stats.lastDate ? ` · última atualização ${parseDateOnly(stats.lastDate).toLocaleDateString("pt-BR")}` : ""}
          </p>
        </div>
        <button
