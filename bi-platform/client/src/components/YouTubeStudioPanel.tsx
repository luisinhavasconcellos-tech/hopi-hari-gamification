      ...
      <div className="rounded-2xl border border-border bg-card p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-[11px] text-muted-foreground">Últimos 28 dias · snapshot {parseDateOnly(totals.snapshot_date).toLocaleDateString("pt-BR")}</p>
          </div>
        </div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={daily}>
              ...
              <XAxis dataKey="date" stroke="hsl(var(--muted-foreground))" fontSize={11}
                tickFormatter={(d) => parseDateOnly(String(d)).toLocaleDateString("pt-BR", { day: "2-digit", month: "2-digit" })} />
              <YAxis stroke="hsl(var(--muted-foreground))" fontSize={11} tickFormatter={(v) => fmt(v)} />
              <Tooltip
                contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8 }}
                labelFormatter={(d) => parseDateOnly(String(d)).toLocaleDateString("pt-BR")}
