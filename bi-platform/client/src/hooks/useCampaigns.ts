...
export function useCampaigns() {
  ...
  const [driveSources, setDriveSources] = useState<DriveCampaignPayload["sources"]>([]);
  const [loading, setLoading] = useState(true);

  // Depende do usuário, não do token: o access_token rota a cada hora e
  // re-disparava o download completo de posts/seguidores.
  ...
  useEffect(() => {
    let alive = true;
    (async () => {
      const accessToken = userId ? sessionRef.current?.access_token : undefined;
      const drivePayloadPromise: Promise<DriveCampaignPayload | null> = accessToken
        ? fetch("/api/campaigns", { headers: { Authorization: `Bearer ${accessToken}` } })
            .then(async response => response.ok ? response.json() as Promise<DriveCampaignPayload> : null)
            .catch(() => null)
      ...
      const { rows: fd } = await fetchAllRows<Record<string, unknown>>((from, to) =>
        supabase
          .from("follower_daily")
      ...
      const drivePayload = await drivePayloadPromise;

      if (!alive) return;
      const merged = new Map<string, CampaignRow>();
      for (const campaign of (camps ?? []) as unknown as CampaignRow[]) {
        merged.set(campaign.summary?.drive_folder_id ?? campaign.id, campaign);
      }
      for (const campaign of drivePayload?.campaigns ?? []) {
        const existing = merged.get(campaign.id);
        merged.set(campaign.id, {
          id: existing?.id ?? campaign.id,
          slug: existing?.slug ?? campaign.slug,
          name: campaign.name,
          brand: existing?.brand ?? campaign.brand,
          period_start: existing?.period_start ?? campaign.periodStart,
          period_end: existing?.period_end ?? campaign.periodEnd,
          source: existing ? `${existing.source} + Google Drive` : campaign.source,
          summary: {
            drive_folder_id: campaign.id,
            folder_url: campaign.folderUrl,
            assets: { total: campaign.assets, images: campaign.images, videos: campaign.videos },
            formats: campaign.formats,
            subfolders: campaign.subfolders,
            previews: campaign.previews,
          },
        });
      }
      setCampaigns([...merged.values()]);
      setDriveSources(drivePayload?.sources ?? []);
      setFollowers(
        fd.map((r) => ({
  ...
  }, [campaigns, posts, followers, driveSources, loading]);
