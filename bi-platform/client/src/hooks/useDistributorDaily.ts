...
export function useDistributorDaily() {
  ...
  return useMemo(() => {
    ...
    // mês corrente = último mês com dados no ano atual (as linhas vêm em ordem crescente)
    const currentMonths = rows.filter((r) => r.year === current).map((r) => r.month);
    const month = currentMonths.length ? Math.max(...currentMonths) : new Date().getMonth() + 1;
    ...
    // um mesmo dia pode ter linha "daily" e "consignado": soma todas as linhas do dia
    const onDay = (l: DistributorDailyRow[], day: number) => {
      const list = l.filter((r) => dayOf(r) === day);
      return list.length ? { revenue: sum(list, "revenue"), quantity: sum(list, "quantity") } : null;
    };

    let curAcc = 0;
    let prevAcc = 0;
    const daily = days.map((day) => {
      const c = onDay(cur, day);
      const p = onDay(prev, day);
      curAcc += c?.revenue ?? 0;
      prevAcc += p?.revenue ?? 0;
      return {
