...
/** Descarta o cache em memória (chamado no logout para não vazar dados entre sessões). */
export function resetParkAttractionsCache() {
  parkCache = null;
  inflight = null;
}
...
async function fetchPark(): Promise<ParkCache> {
  const [a, p] = await Promise.all([
    fetchAllRows<AttractionRow>((from, to) =>
      supabase
    ...
    fetchAllRows<ParkPublicRow>((from, to) =>
      supabase
