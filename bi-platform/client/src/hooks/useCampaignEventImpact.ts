...
export function useCampaignEventImpact(campaigns: EnrichedCampaign[]) {
  ...
  useEffect(() => {
    let alive = true;
    (async () => {
      const [{ data: ev }, { rows: rides }, { rows: sales }] = await Promise.all([
        supabase
          .from("park_events")
        ...
        fetchAllRows<{ year: number; month: number; rides: number | null }>((from, to) =>
          supabase
            .from("park_attraction_monthly")
            .select("year, month, rides")
            .order("year")
            .order("month")
            .order("id")
            .range(from, to),
        ),
        fetchAllRows<{ year: number; month: number; revenue: number | null; quantity: number | null }>((from, to) =>
          supabase
