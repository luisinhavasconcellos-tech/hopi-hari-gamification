...
export function useReputationCorrelation(
  ...
  /** carrega sempre 12 meses (+12 de janela anterior, via useReputation) para comparar sem refetch */
  ...
  useEffect(() => {
    ...
    (async () => {
      const [attr, leads] = await Promise.all([
        fetchAllRows<{ area: string; rides: number }>((from, to) =>
          supabase.from("park_attraction_monthly").select("area, rides").order("id").range(from, to),
        ),
