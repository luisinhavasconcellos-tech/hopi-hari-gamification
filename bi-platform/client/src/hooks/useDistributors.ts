  ...
  useEffect(() => {
    ...
    (async () => {
      const [d, s] = await Promise.all([
        fetchAllRows<Distributor>((from, to) =>
          supabase
            .from("distributors")
            .select("full_key, name, code, region, phone, email")
            .order("name")
            .order("id")
            .range(from, to),
        ),
        fetchAllRows<DistributorSale>((from, to) =>
          supabase
      ...
      if (!alive) return;
      setDistributors(d.rows);
      setSales(s.rows.map((r) => ({ ...r, revenue: Number(r.revenue) })));
      setLoading(false);
    })();
    return () => {
      alive = false;
