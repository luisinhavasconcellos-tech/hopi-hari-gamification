...
export function correlationTest(a: number[], b: number[], minN = 4): CorrelationTest {
  // descarta pares com NaN/Infinity — um único valor não finito contaminava r, p e IC
  ...
  const ma = pairs.reduce((s, [v]) => s + v, 0) / n;
  const mb = pairs.reduce((s, [, v]) => s + v, 0) / n;
  let num = 0;
  let da = 0;
  let db = 0;
  for (let i = 0; i < n; i++) {
    const x = pairs[i][0] - ma;
    const y = pairs[i][1] - mb;
    num += x * y;
    da += x * x;
    db += y * y;
  }
