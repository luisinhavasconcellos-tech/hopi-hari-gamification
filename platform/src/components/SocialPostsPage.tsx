...
export default function SocialPostsPage({ platform }: { platform: SocialPlatform }) {
  ...
  const loadFollowers = async () => {
    setFollowers(await fetchFollowerCount(platform));
  };
  ...
  const syncSheet = async (silent = false) => {
    ...
    try {
      ...
      toast({ title: "Erro ao sincronizar planilha", description: e instanceof Error ? e.message : "Falha", variant: "destructive" });
    }
    setSyncing(false);
  };
