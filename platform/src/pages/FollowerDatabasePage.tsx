...
export default function FollowerDatabasePage() {
  ...
  const model = useMemo(() => {
    ...
    const growthPerWeek = (key: PlatformKey | "total") => {
      ...
      const endTs = new Date(end.date).getTime();
      const target = endTs - 28 * 86400000;
      let start = arr[0];
      for (const a of arr) if (new Date(a.date).getTime() <= target) start = a;
      const days = (endTs - new Date(start.date).getTime()) / 86400000;
    ...
    <div className="min-h-screen p-4 sm:p-6 lg:p-8">
      <div className="flex items-start justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            ...
            {model.lastDate ? ` · atualizado em ${new Date(model.lastDate).toLocaleDateString("pt-BR")}` : ""}
          </p>
        </div>
        <button
          onClick={sync}
      ...
      {rows.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-border p-10 text-center text-sm text-muted-foreground">
          Nenhum dado de seguidores. Clique em “Sincronizar planilha”.
        </div>
      ) : (
