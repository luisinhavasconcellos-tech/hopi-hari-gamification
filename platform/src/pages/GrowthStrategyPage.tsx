...
export default function GrowthStrategyPage() {
  ...
  const [tab, setTab] = useState<"global" | "strategy" | "bots">("global");
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4">
        ...
        <p className="text-muted-foreground text-sm">Gerando estratégia personalizada com IA...</p>
  ...
  return (
    <div className="min-h-screen p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex gap-2 mb-6">
        {(["global", "strategy", "bots"] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)} className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${tab === t ? "bg-primary/20 text-primary border border-primary/30" : "bg-card text-muted-foreground border border-border hover:text-foreground"}`}>
            {t === "global" ? "Relatório Global (IA)" : t === "strategy" ? "Estratégia" : "Detector de Bots"}
          </button>
        ))}
      </div>
        ...
        <div className="space-y-6">
          ...
          {scrapedCount === 0 && (
            <div className="bg-warning/10 border border-warning/30 rounded-2xl p-5 text-sm text-warning-foreground">
              Nenhum post processado ainda. Vá para <a href="/posts" className="text-primary underline">Posts</a> e inicie a coleta de dados primeiro.
            </div>
          )}
