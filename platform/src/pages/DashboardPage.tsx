...
import { fetchAccountMetrics, fetchAccountInsights, fetchRecentMedia, fetchFollowerGrowth } from "@/lib/instagram";
    ...
    (async () => {
      try {
        if (hasRealToken) {
          ...
          const igId = localStorage.getItem("ig_user_id") ?? "";
          const [acc, ins, meds, grw] = await Promise.all([
            fetchAccountMetrics(token, igId),
            fetchAccountInsights(token, igId),
            fetchRecentMedia(token, igId, 6),
            fetchFollowerGrowth(token, igId),
          ]);
          setAccount(acc);
          setInsights(ins);
          setPosts(meds);
        ...
        // Always load real posts from DB regardless of token
    ...
    <div className="min-h-screen p-4 sm:p-6 lg:p-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatCard
          ...
          sub={`+${growthDelta.toLocaleString("pt-BR")} em 30 dias`}
          accent
        />
        <StatCard
