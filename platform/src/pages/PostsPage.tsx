...
export default function PostsPage() {
  ...
  const loadFollowers = async () => {
    setFollowers(await fetchFollowerCount("instagram"));
  };
  ...
  const syncSheet = async (silent = false) => {
    ...
    try {
      ...
      toast({ title: "Erro ao sincronizar", description: e instanceof Error ? e.message : "Falha", variant: "destructive" });
    }
    setSyncing(false);
  };
  ...
  useEffect(() => { (async () => { await Promise.all([loadPosts(), loadFollowers()]); await syncSheet(true); })(); }, []);
