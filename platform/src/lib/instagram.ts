...
export async function exchangeCodeForToken(code: string): Promise<string> {
  const res = await fetch(
    `${FB_BASE}/oauth/access_token?client_id=${import.meta.env.VITE_INSTAGRAM_APP_ID}&redirect_uri=${import.meta.env.VITE_REDIRECT_URI}&client_secret=${import.meta.env.VITE_INSTAGRAM_APP_SECRET}&code=${code}`
  );
...
export async function getLongLivedToken(shortToken: string): Promise<string> {
  const res = await fetch(
    `${FB_BASE}/oauth/access_token?grant_type=fb_exchange_token&client_id=${import.meta.env.VITE_INSTAGRAM_APP_ID}&client_secret=${import.meta.env.VITE_INSTAGRAM_APP_SECRET}&fb_exchange_token=${shortToken}`
  );
  const data = await res.json();
  return data.access_token;
}
