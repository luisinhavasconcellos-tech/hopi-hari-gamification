...
export async function generateDailyReportPdf(opts: { withAi?: boolean } = {}) {
  ...
  // ----- Novos Seguidores · Últimos 7 dias (estilo dashboard) -----
  {
    ...
    for (const p of PLATFORMS) {
      ...
      let sum7 = series
        .filter((d) => new Date(d.date).getTime() > cutoff7)
        .reduce((s, d) => s + (d.new_followers || 0), 0);
      let sum1 = series
        .filter((d) => new Date(d.date).getTime() > cutoff1)
        .reduce((s, d) => s + (d.new_followers || 0), 0);
      ...
      if (sum7 === 0 && withFollowers.length >= 2) {
        const findOnOrBefore = (ts: number) => {
          ...
          for (const r of withFollowers) {
            if (new Date(r.date).getTime() <= ts) pick = r;
            else break;
          }
          return pick;
        };
    ...
    const headerLabel = headerDate
      ? new Date(headerDate as string).toLocaleDateString("pt-BR")
      : new Date().toLocaleDateString("pt-BR");
  ...
  // ----- Crescimento Diário de Seguidores (por plataforma) -----
  {
    ...
    autoTable(doc, {
      startY: y + 2,
      head: [["Plataforma", "Total atual", "Δ Diário", "Δ 7 dias", "Δ 30 dias", "Última atualização", "Dias c/ dado"]],
      body: rowsBody,
