    ...
    (async () => {
      const [r, p] = await Promise.all([
        supabase
          ...
          .order("year")
          .order("month"),
        supabase.from("sales_product_yearly").select("channel, product, year, quantity, revenue"),
      ]);
      if (!alive) return;
      setRows(((r.data ?? []) as RevenueRow[]).map((x) => ({ ...x, quantity: Number(x.quantity), revenue: Number(x.revenue) })));
      setProducts(((p.data ?? []) as ProductRow[]).map((x) => ({ ...x, quantity: Number(x.quantity), revenue: Number(x.revenue) })));
      setLoading(false);
    })();
    return () => {
      alive = false;
