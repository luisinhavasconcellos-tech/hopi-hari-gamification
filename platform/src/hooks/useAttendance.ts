  ...
  useEffect(() => {
    ...
    (async () => {
      const [pm, fc, sr] = await Promise.all([
        supabase.from("park_public_monthly").select("year, month, visitors, open_days").order("year").order("month"),
        supabase.from("park_visitors_forecast").select("date, visitors").order("date"),
        supabase
      ...
      if (!alive) return;
      setPublicMonthly(
        ((pm.data ?? []) as PublicMonth[]).map((r) => ({ ...r, visitors: Number(r.visitors), open_days: r.open_days })),
      );
      setForecast(((fc.data ?? []) as ForecastDay[]).map((r) => ({ ...r, visitors: Number(r.visitors) })));
      setTicketSales(
        ((sr.data ?? []) as RevenueMonth[]).map((r) => ({
          ...r,
